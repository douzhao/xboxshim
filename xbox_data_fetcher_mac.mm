#include "xbox_data_fetcher_mac.h"
...
XboxDataFetcher::XboxDataFetcher(id<XboxDeviceEventDelegate> delegate)
    : delegate_(delegate),
      listening_(false),
      source_(NULL),
      port_(NULL) {
...
void XboxDataFetcher::DeviceRemoved(void* context, io_iterator_t iterator) {
  ...
  while ((ref = IOIteratorNext(iterator))) {
    base::mac::ScopedIOObject<io_service_t> scoped_ref(ref);
    base::ScopedCFTypeRef<CFNumberRef> number((CFNumberRef) IORegistryEntryCreateCFProperty(ref,
...
bool XboxDataFetcher::RegisterForDeviceNotifications(
  ...
  if (ret != kIOReturnSuccess) {
    NSLog(@"Error listening for Xbox controller add events: %d",ret);
    return false;
  }
  ...
  if (ret != kIOReturnSuccess) {
      NSLog(@"Error listening for Xbox controller remove events: %d", ret);
    return false;
  }
...
void XboxDataFetcher::AddController(XboxController* controller) {
  if (ControllerForLocation(controller->location_id()))
      NSLog(@"Controller with location ID %d already exists in the set of controllers.",controller->location_id());
...
void XboxDataFetcher::XboxControllerGotData(XboxController* controller,
    ...
    [delegate_ XboxValueChanged: controller data: data];
}
